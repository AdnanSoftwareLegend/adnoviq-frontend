  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-slate-50 px-4 py-16 dark:bg-zinc-950">
      <div className="w-full max-w-md rounded-lg border border-slate-200 bg-white p-8 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
        ...
        <p className="mt-2 text-sm text-slate-600 dark:text-zinc-400">Sign in to continue shopping and manage your account.</p>
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-4">
          ...
          <input type="password" value={password} onChange={(e) => { clearError(); setPassword(e.target.value); }} className="w-full rounded-lg border border-slate-200 bg-white px-4 py-3 text-slate-900 outline-none transition focus:border-orange-500 focus:ring-2 focus:ring-orange-100 dark:border-zinc-800 dark:bg-zinc-950 dark:text-white" placeholder="Password" autoComplete="current-password" />
        ...
        <p className="mt-4 text-sm text-slate-600 dark:text-zinc-400">
          No account yet? <Link href="/register" className="font-semibold text-orange-600">Create one</Link>
        </p>
      </div>
    </main>
  );
